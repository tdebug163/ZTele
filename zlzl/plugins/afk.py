...
async def set_not_afk(event):
    ...
    # حساب الوقت بطريقة بسيطة جداً عشان ما يضرب
    endtime = "وقت غير معروف"
    if AFK_.afk_star:
        diff = datetime.now().replace(microsecond=0) - AFK_.afk_star
        endtime = str(diff)
        ...
        if BOTLOG and BOTLOG_CHATID:
            try:
                await event.client.send_message(
                    BOTLOG_CHATID,
                    "⪼  انتهاء امر السليب \n"
                    + "`⪼  تم تعطيله والرجوع للوضع الطبيعي`",
                )
            except:
                pass
